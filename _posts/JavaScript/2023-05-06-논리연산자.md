...
{
  !!GetMyInfoError && "data" in GetMyInfoError && (
    <Alert
      message={`내 정보를 가져오는데 실패했습니다.`}
      description={JSON.stringify(GetMyInfoError?.data)}
      type="error"
      closable
    />
  );
...
`!!GetMyInfoError` 가 true 이고, `"data" in GetMyInfoError` 가 true 이면, 그 뒤의 jsx 를 실행한다. 즉 렌더된다는 뜻. 왜냐? `&&` 비교연산자는 falsy 한값을 찾는데, 없으면 마지막 피연산자를 실행하기 때문! 일반적으로 사용해 왔지만, 하나의 문법이라고 외우고 사용했었다.
